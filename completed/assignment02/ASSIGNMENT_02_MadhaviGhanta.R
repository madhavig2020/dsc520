...
##not necessary, since current directory has data folder

## Load the file `data/tidynomicon/person.csv` to `person_df1` using `read.csv`
## Examine the structure of `person_df1` using `str()`
person_df1 <- read.csv("c:/users/mghan/Documents/dsc520/data/tidynomicon/person.csv")
str(person_df1)
...
#person_df2 <-  read.csv("c:/users/mghan/Documents/dsc520/data/tidynomicon/person.csv" ,stringsAsFactors = FALSE )
#str(person_df2)
...
scores_df <-  read.csv(file = "c:/users/mghan/Documents/dsc520/data/tidynomicon/person.csv")
summary(scores_df)
...
voter_turnout_df2 <- read_excel('data/G04ResultsDetail2004-11-02.xls',sheet='Voter Turnout',
                                col_names = c("ward_precinct","ballots_cast",
                                              "registered_voters","voter_turnout"),skip=2)
